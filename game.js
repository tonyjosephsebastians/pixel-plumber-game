(() => {
  ...
  class Game {
    ...
    loadLevel(i) {
      this.levelIdx = i;
      this.level = cloneData(levelData[i]);
    ...
    update() {
      ...
      const targetCamera = Math.max(0, Math.min(this.level.width * TILE - WIDTH, p.x - WIDTH * 0.38));
    ...
    overlay() {
      ...
      if (this.state === "start") {
        ...
        ctx.fillStyle = "#66f0d6";
        ctx.fillText("PIXEL PLUMBER ADVENTURE", WIDTH / 2, 155);
        ctx.fillStyle = "#ffffff";
        ctx.font = "22px monospace";
        ctx.fillText("RUN • JUMP • COLLECT • SURVIVE", WIDTH / 2, 205);
        ctx.fillText("Move: A/D or Arrow Keys", WIDTH / 2, 255);
        ctx.fillText("Jump: W / Up / Space", WIDTH / 2, 290);
        ctx.fillText("Pause: P   Restart: R", WIDTH / 2, 325);
        ctx.fillStyle = "#ffe082";
        ctx.fillText("Press Enter to Start", WIDTH / 2, 385);
